$(function() {		
	...
	JQTWEET = {
			...
			template: '<div class="item">{IMG}<div class="tweet-wrapper"><span class="text">{TEXT}</span>\
				...
				by <span class="user showUserTwitter cursorP" data-screenName="{showUserTwitter}">{USER}</span></div>\
                <i class="fa fa-retweet text-primary" data-rewteetId="{RETWEETID}" title="Retweet"></i>\
                <i class="fa fa-off-star text-primary" data-favouriteId="{FAVOURITEID}" title="Preferiti"></i>\
                <i class="fa fa-mail-reply text-primary" data-replayTweetId="{REPLAYTWEETID}" data-replayTweetuser="{REPLAYTWEETUSER}" title="Rispondi"></i>\
            </div>',
			...
			loadTweets: function() {
				...
				$.ajax({
					...
					url: ajaxCallPath + 'call.php?action=getUserTimelineTweet',
					type: 'POST',
					dataType: 'json',
					data: request,
					success: function(data, textStatus, xhr) {
                        
                        $( 'body' ).on( 'click', '[data-rewteetId]', function() {
                            twitter.sendRetweet( this );
                        });
                        $( 'body' ).on( 'click', '[data-replayTweetId]', function() {
                            twitter.replayTweetId( this );
                        });
						
						$(JQTWEET.appendTo).empty();
                        
						if (data.httpstatus == 200 || 1==1) {
							if (JQTWEET.search) data = data.statuses;

							var text, name, img;
                            
                            JQTWEET.numTweets = Object.keys( data ).length;
							...
							try {
								...
								{		
                                    ...
                                    console.info( data[i].favorited );
                                    if( data[i].favorited === true ) {
                                        console.info( 'entra' );
                                        JQTWEET.template = JQTWEET.template.replace('fa-off-star', 'fa-star' );
                                    } else {
                                        JQTWEET.template = JQTWEET.template.replace('fa-star', 'fa-off-star' );
                                    }
                                    
                                    
									$(JQTWEET.appendTo).append( JQTWEET.template.replace('{TEXT}', JQTWEET.ify.clean(data[i].text) )
										.replace('{USER}', data[i].user.screen_name)
										.replace('{showUserTwitter}', data[i].user.screen_name)
										.replace('{RETWEETID}', data[i].id_str)
										.replace('{REPLAYTWEETID}', data[i].id_str)
										.replace('{FAVOURITEID}', data[i].id_str)
										.replace('{REPLAYTWEETUSER}', data[i].user.screen_name)
										.replace('{IMG}', img)                                
