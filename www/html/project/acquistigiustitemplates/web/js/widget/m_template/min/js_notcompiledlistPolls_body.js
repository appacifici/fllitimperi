 ...
 * Metodo che avvia gli ascoltatori
 */
Skin.prototype.initListeners = function() {
    var that = this;      
    
    this.intervalCheck = setInterval( function() {
        that.detectWidthContainer();
        that.numberCheck++;
    },500 );    
};

/**
 * Determina se è stato caricato lo skin e adatta il sito
 * @returns {undefined}
 */
Skin.prototype.detectWidthContainer = function() {    
    if( this.numberCheck == 20 ) {
        clearInterval( this.intervalCheck );
    }
        
    if( $( 'body' ).css( 'background' ).indexOf( 'http' ) != '-1' ) {        
        clearInterval( this.intervalCheck );
        $( '.widget_TopMenu .nav-account:nth-child(2)' ).hide();
        $( '.widget_Menu .header-logo' ).remove();
        $( '.widget_Menu .container' ).css( 'marginLeft', 0);
        $( '.header__secondary .header-search-form' ).remove();
        $( '.header__primary-inner' ).addClass('skinHeader__primary-inner');        
        $('.site-content .container').css('marginRight', '2px');
        $('.site-content .container').css('marginLeft', '2px');
        $('.hero-slider, .hero-slider__item').css('height', '535px');
        $('.hero-slider__item-container').css('top', '35%');
        $('.hero-slider__item-container').css('marginRight', '3%');
        $('.hero-slider__item-container').css('marginLeft', '3%');
                
        if( $('body').width() > 1440 ) {
//            $('body').css('backgroundSize', 'cover');
            $('.container').css('maxWidth', '1000px');
            $('#site-wrapper').css('maxWidth', '1000px');
            $('#site-wrapper').css('margin', '0 auto');
            $('.site-content .container').css('maxWidth', '985px');
        } else if( $('body').width() <= 1280 ) {
            $('.container').css('maxWidth', '980px');
            $('#site-wrapper').css('maxWidth', '980px');
            $('#site-wrapper').css('margin', '0 auto');
            $('.site-content .container').css('maxWidth', '980px');
 ...
 * Classe per la gestione del widget ImageArticle
 */
var WidgetInfiniteScrollPolls = function () {
    this.widget            = $( ".widget_PollsGrid" );
    this.initListeners();
};
 ...
 * Metodo che avvia gli ascoltatori
 */
WidgetInfiniteScrollPolls.prototype.initListeners = function () {
    var that = this;
    this.page = 2;
    this.lock = false;
    this.lastScroll = 0;
...
WidgetInfiniteScrollPolls.prototype.scroll = function ( tollerance ) {
    var that = this;    
    ...
    $(window).scroll(function (event) {
        ...
        if( ( $(window).scrollTop() + $(window).height() + tollerance > $(document).height() ) && $(document).height() != that.lastScroll ) {                     
            ...
            var request = $.ajax ({
                url: "/news/getInfiniteScrollerListPolls/page="+that.page+"?uri="+window.location.pathname+'&'+window.location.search.replace('?',''),
                type: "GET",
                async: true,
                dataType: "html"        
             });
            request.done( function( resp ) {
                ...
                $('.widget_PollsGrid:last-child').after( resp );
...
widgetInfiniteScrollPolls = null;

WidgetInfiniteScrollPolls = new WidgetInfiniteScrollPolls();
