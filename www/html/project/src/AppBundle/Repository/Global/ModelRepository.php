...
class ModelRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    public function getAllModelsFosElastica() {
       $repository = $this->getEntityManager()->getRepository('AppBundle:Model');
    ...
    public function getCountModelHasComparison() {
        $query = $this->getEntityManager()
                ->createQuery(
                'SELECT COUNT(m) as tot FROM AppBundle:Model m WHERE m.comparisonModels IS NOT NULL');
    ...
    public function getEmptyProductsModel() {
       $query = $this->getEntityManager()
                ->createQuery(
                'SELECT m FROM AppBundle:Model m WHERE m.hasProducts = 0');  
    ...
    public function getModelHasComparison( $limit='0,100') {
        $limit = explode(',', $limit);
        ...
        $query = $this->getEntityManager()
                ->createQuery(
                'SELECT m FROM AppBundle:Model m WHERE m.comparisonModels IS NOT NULL')->setFirstResult($firstResult)->setMaxResults($lastResult);  
...
//        
        ...
        $sql = 'SELECT m,tr,s,t,c FROM AppBundle:Model m                          
                left JOIN m.trademark tr                 
                left JOIN m.subcategory s                 
                left JOIN m.typology t                 
                left JOIN m.category c                 
                WHERE '. $where .' 
                    and m.isCompleted=1 and m.inShowcase = 1
            ORDER BY m.lastReadPrice DESC';
    ...
    public function findModelsWithLimit( $limit, $name = false, $isCompleted = false, $category = false, $subcategory = false, $typology = false, $inShowcase = false, $revisioned = false, $date = false, $top = false ) { 
        ...
        $query = $this->getEntityManager()
                ->createQuery(
                'SELECT m,tr,s,t,c FROM AppBundle:Model m                          
                        left JOIN m.trademark tr                 
                        left JOIN m.subcategory s                 
                        left JOIN m.typology t                 
                        left JOIN m.category c
...
//        echo $query->getSQL();
    ...
    public function findCountModelsBySubcategoryTypologyTrademark( $subcategoryId = false, $typologyId = false, $trademarkId, $isCompleted = false  ) {         
        ...
        $sql = 'SELECT  COUNT(m) as tot  FROM AppBundle:Model m
...
//        echo $query->getSQL();        
    ...
    public function findDistinctNameUlrTrademarkModelSubcategoryTypology( $subcategoryId = false, $typologyId = false  ) {         
        ...
        $sql = 'SELECT DISTINCT t.nameUrl FROM AppBundle:Model m
            JOIN m.trademark t
            where '.$where2.$where.'
        ';
...
//        echo $query->getSQL ();
    ...
    public function findDistinctIdNameTrademarkModelSubcategoryTypology( $subcategoryId = false, $typologyId = false  ) {         
        ...
        $sql = 'SELECT DISTINCT t.id, t.name FROM AppBundle:Model m
            JOIN m.trademark t
            '.$where.' 
        ';
     ...
     * @return type
     */
    public function findAllBySubcategoryTypology( $subcategoryId = false, $typologyId = false  ) {         
        $where = 'where m.isActive = 1 and m.isCompleted=1';
        if( !empty( $subcategoryId ) ) {
            $where .= " and m.subcategory = $subcategoryId";
        }
        if( !empty( $typologyId ) ) {
        ...
        $sql = 'SELECT m FROM AppBundle:Model m
            '.$where.' ORDER BY m.dateRelease DESC
        ';
        ...
        if( !SECOND_LEVEL_CACHE_ENABLED || !$query->isCacheable() ) {
            $query->useQueryCache( true );
            $query->useResultCache(true, BESTSELLER_MODEL_REGION_TTL, 'model_findAllBySubcategoryTypology_CacheORM'.md5( $sql ) );    
        }    
    ...
    public function countModels( $name = false, $isCompleted = false, $category = false, $subcategory = false, $typology = false, $inShowcase = false, $revisioned = false, $date = false, $top = false ) {
        ...
        $sql = 'SELECT COUNT(m) as tot FROM AppBundle:Model m '.$where;        
        $query = $this->getEntityManager()->createQuery( $sql );
...
//        $lastResult = count($limit) == 1 ? $limit[0] : $limit[1];
        ...
        if( $date !== false ) {
            $dateInit = date( 'Y-m-d ', strtotime( str_replace( ' ', '-', $date ) ) ). '00:00:00';
            $where .= " and m.dateRelease > '$dateInit' ";
        }
        ...
        $query = $this->getEntityManager()
                ->createQuery(
                        ...
                        left JOIN m.typology t                 
                        left JOIN m.category c      
                         WHERE m.price > 0  and ( m.price < m.lastPrice )  '.$where.' ORDER BY m.lastReadPrice DESC')->setMaxResults($limit);
...
//        
        ...
        $query = $this->getEntityManager()
                ->createQuery(
                'SELECT COUNT(m) as tot FROM AppBundle:Model m
                WHERE '. $where.'' );
     ...
     * Query che ritorna tutti i prodotti più venduti
     */
    public function getModelsList( $limit, $top, $category, $subcategory, $typology, $microSection = false, $order = false, $trademark = false, $affiliate = false, $aPrice = array() ) {
        $where = 'm.isActive = 1 and m.isCompleted = 1';
        
        $order = !empty( $order ) ? $order : 'm.dateRelease DESC' ;
