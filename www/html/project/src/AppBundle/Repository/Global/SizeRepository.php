...
class SizeRepository extends \Doctrine\ORM\EntityRepository
{
     ...
     * @return type
     */
    public function findAllSizes( $toObject = false ) {
        $sql = 'SELECT a FROM AppBundle:Size a ORDER BY a.name ASC';        
        $query = $this->getEntityManager()->createQuery( $sql );
        
        $query->setCacheable( true );
        $query->setCacheRegion( 'my_sizes_region_query' );
                
        if( !SECOND_LEVEL_CACHE_ENABLED || !$query->isCacheable() ) {
            $query->useQueryCache( true );
            $query->useResultCache(true, CATEGORY_REGION_TTL, 'size_findAllSizes_CacheORM'.md5( $sql ) );    
        }        
        ...
        if( SECOND_LEVEL_CACHE_ENABLED ||  SECOND_LEVEL_CACHE_SET_EXPIRE_ENABLED  )
            $this->secondLevelCacheUtility->setExpire( 'size', CATEGORY_REGION_TTL );
