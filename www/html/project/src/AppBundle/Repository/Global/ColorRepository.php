...
class ColorRepository extends \Doctrine\ORM\EntityRepository
{
     ...
     * @return type
     */
    public function findAllColors( $toObject = false ) {
        $sql = 'SELECT a FROM AppBundle:Color a ORDER BY a.name ASC';        
        $query = $this->getEntityManager()->createQuery( $sql );
        
        $query->setCacheable( true );
        $query->setCacheRegion( 'my_colors_region_query' );
                
        if( !SECOND_LEVEL_CACHE_ENABLED || !$query->isCacheable() ) {
            $query->useQueryCache( true );
            $query->useResultCache(true, CATEGORY_REGION_TTL, 'color_findAllColors_CacheORM'.md5( $sql ) );    
        }        
