...
use AppBundle\Service\GlobalConfigService;
...
class TemplateController extends Controller {
    ...
    public function getVersionSite() {
        $this->globalConfigManager = $this->get('app.globalConfigManager');
     ...
     */
    public function baseParameters() {
...
//        $this->skin                 = $this->globalConfigManager->getSkin();
                
        $versioneDependency = ucfirst( str_replace( array( 'app_', 'm_', 'amp_' ), '', $this->versionSite ) );
        if( 'admin' == strtolower($versioneDependency) )
            $this->dependencyManager = $this->get('app.dependencyManagerAdmin' );
        else
            $this->dependencyManager = $this->get('app.dependencyManagerTemplate' );
    ...
    public function setParameters( $extraParams = false ) {      
        ...
        $this->folderCss = $this->container->getParameter('app.folder_css') . $this->versionSite;
        $this->folderJs = $this->container->getParameter('app.folder_js') . $this->versionSite;
        $this->folderJsMin = $this->container->getParameter('app.folder_js_minified'). $this->versionSite;
        $this->compactSite = $this->container->getParameter('app.compactSite');
        
        $this->compactSite =  $this->versionSite == 'admin' ? false : $this->container->getParameter('app.compactSite');
        $this->extension = $this->container->getParameter('app.extensionTpl');

        
        if (!empty($this->compactSite)) {
            $this->folderCss = $this->container->getParameter('app.cdn') . $this->folderCss . $this->container->getParameter('app.compactVersion');
            $this->folderJs = $this->container->getParameter('app.cdn') . $this->folderJsMin . $this->container->getParameter('app.compactVersion');
        }
       
        
        $this->get('twig')->addGlobal('versionSite', $this->versionSite);
        $this->get('twig')->addGlobal('isMobile', $this->isMobile);
        $this->get('twig')->addGlobal('cdn', $this->container->getParameter('app.cdn'));
    }
     ...
     */
    public function init($fileStructure, Request $request, $extraParams = null) {  
        ...
        $this->get('twig')->addGlobal('date', $date);
        ...
        $tplViewTplSessionActive = array(
            'widget_UserMenu' => 'widget_ButtonLogin'
        );
...
////        return $this->render( $this->versionSite.'/_index'.$this->extension, array(
        $html = $this->get('twig')->render( $this->versionSite.'/_index'.$this->extension, array(
            'appConfig'         => $this->container->getParameter( 'app.js' ),
            'compactSite'       => $this->compactSite,
            'compactVersion'    => $this->container->getParameter( 'app.compactVersion' ),
...
//        return $html;
    ...
    function createTemplate($xml) {
        $this->get('twig')->addGlobal('dependenciesCSSHead', '');
        $this->get('twig')->addGlobal('dependenciesCSSBody', '');
        $this->get('twig')->addGlobal('dependenciesJSHead', '');
        $this->get('twig')->addGlobal('dependenciesJSBody', '');
        $this->get('twig')->addGlobal("fetch_index", '');
        ...
        foreach ($xml->tpl as $node) {
            ...
            $this->get('twig')->addGlobal("fetch_" . $name[0], '');

            if (!empty($node['assign'])) {
                $this->get('twig')->addGlobal("fetch_" . $node['assign'], '');
            }
        ...
        if (strpos(strtolower($template), "tructure_") || strpos(strtolower($template), "idget_")|| strpos(strtolower($template), "anner_")) {
            ...
            if (( empty($ajaxCore) || $ajaxCore == 0 ) && !empty($cores)) {
                        ...
                        if( !empty( $data_new['errorPage'] ) && !empty( $this->responsePage ) ) {
                            switch( $data_new['errorPage'] ) {                                
                                case 404:
                                    ...
                                    return $this->get('twig')->render($this->versionSite . "/widget_404NotFound" . $this->extension, $data);
                                break;
                                case 'notResultSearchFoundPage':
                                    $widget = $this->get('app.CoreBestsellerModels' );
                                    $data = $widget->processData( $options );                                    
                                    
                                    $this->responsePage->setStatusCode(200);                            
                                    $this->get('twig')->addGlobal('error404Page', true );
                                    return $this->get('twig')->render($this->versionSite . "/widget_NotResultSearchFound" . $this->extension, $data);
                                break;
                            }                           
                        }
                ...
                if( in_array( 'CoreReplaceWidgetAjax', $cores ) ) {
                    $coreAjaxReplaceDataAttr = $this->get('app.CoreReplaceWidgetAjax' );
                    $options = new \stdClass();                    
                    $options->varAttrAjax   = $varAttrAjax; 
            ...
            $resp = $this->get('twig')->render($this->versionSite . "/{$template}" . $this->extension, $data);
            return $resp;
        }
        return '';
    }
...
//        return $response;
        ...
        if( !empty( $enabledCache ) && !empty( $extraConfig['cacheEnabled']->getValue() ) && $extraConfig['cacheEnabled']->getValue() == 'true' ) {    
            ...
            if ($response->isNotModified($request)) {
                ...
                $response->setETag( $eTag );
                $response->setPublic();
                $response->setMaxAge( $this->container->getParameter('app.ttlHomaPageCache') );
                $response->setSharedMaxAge( $this->container->getParameter('app.ttlHomaPageCache') );
    ...
    public function renderSingleTemplate(Request $request) {
        ...
        return $this->compressHtml( $this->get('twig')->render($this->versionSite . "/{$template}" . $this->extension, $data ) );
    }
    ...
    public function checkSetResponseCache(Request $request) {
        ...
        if (    !empty($request->query->get('date')) && ( date_format(date_create($request->query->get('date')), 'Y-m-d') < date('Y-m-d') ) 
            ...
            || $this->getEnabledWidgetCached($request->query->get('widget'))
            || $this->getEnabledRouteCached( $request )
        ) {
            return true;
        } else {
            return false;
    ...
    public function getTTLCacheResponse($widget) {
        ...
        if (in_array($widget, $widgetDefaultTTL)) {
            return $this->container->getParameter('s_memcached_expire_default');
        }
        $datWidget = array('widget_TabMenuMatch');
        if (in_array($widget, $datWidget)) {
            return $this->container->getParameter('s_memcached_expire_data_widget');
        }
        return $this->container->getParameter('s_memcached_expire_widget');
    }
    ...
    public function getETagRequest( $widget, $uri ) {
        ...
        if( $widget == 'widget_LiveScoreHome' ) {
            $versionSite = $this->getVersionSite();          
        }
        $eTag = md5( $versionSite.$uri ).'?v='.$this->container->getParameter( 'app.eTagVersion' );
        return $eTag;
    }
