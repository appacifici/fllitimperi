...
class EscarpeAffiliation extends Affiliations {
...
//		$this->product->fkTrademark = $this->getTrademark( $this->product->brand );
	...
	public function mapFields() {
        ...
        if( is_object( $this->product->shippingHandlingCosts ) && empty( $this->product->shippingHandlingCosts->{0})) {
            $this->product->shippingHandlingCosts = null;
        }
...
//		}
        ...
        $this->product->gender = '';
		if ( !empty( $this->product->fields->Plec ) ) {
			$this->product->sex = $this->product->fields->Plec;
		}
	...
	private function insertBridgeData() {
		$this->product->colors = !empty( $this->product->fields->Kolor ) ? $this->product->fields->Kolor : false;
		$this->product->sizes = !empty( $this->product->Rozmiar ) ? $this->product->Rozmiar : false;		
	}
}
