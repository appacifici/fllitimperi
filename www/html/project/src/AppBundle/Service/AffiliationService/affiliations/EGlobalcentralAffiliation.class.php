...
class EGlobalcentralAffiliation extends Affiliations {
...
//		$this->product->fkTrademark = $this->getTrademark( $this->product->brand );
	...
	public function mapFields() {
		...
		$this->product->shippingHandlingCosts = !empty( $this->product->delivery_cost ) && is_null( $this->product->delivery_cost ) ? $this->product->delivery_cost  : 0;
	...
	public function getMerchantCategory() {
		...
		$this->product->nameSubcategory = trim(  str_replace( ">", '-', $this->product->merchant_product_category_path ) );
