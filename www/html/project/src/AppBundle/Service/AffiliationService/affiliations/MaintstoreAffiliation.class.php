...
class MaintstoreAffiliation extends Affiliations {
...
//		$this->product->fkTrademark = $this->getTrademark( $this->product->brand );
	...
	public function getMerchantCategory() {
        if( !empty( $this->product->description ) && !is_object($this->product->description) )
            $this->product->description = str_replace( "''''", '', $this->product->description );
//		$data = explode( '-', $this->product->name );
        
        $ids = false;        
        if( !is_object( $this->product->merchant_product_category_path ) ) {            
            $this->product->nameSubcategory = trim(  str_replace( ">", '-', (string)$this->product->merchant_product_category_path ) );
